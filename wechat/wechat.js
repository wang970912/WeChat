...
const util = require('util'); //用于处理占位符
const accessTokenJson = require('./accessToken.json'); //引入actoken
const menus = require('./menus'); //引入微信菜单配置
const urltil = require('url'); //引入url模块
		...
		if (accessTokenJson.access_token === "" || accessTokenJson.expires_time < currentTime) {
			that.requestGet(url).then(function(data) {
				...
				if (data.indexOf("errcode") < 0) {
					...
					//更新本地存储的
					fs.writeFile('./wechat/accessToken.json', JSON.stringify(accessTokenJson), function(err) {
						if (err) {
							console.error(err);
						} else {
							console.log('写入成功');
